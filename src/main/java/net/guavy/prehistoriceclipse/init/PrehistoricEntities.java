...
public class PrehistoricEntities {
    ...
    public static <T extends DinosaurEntity> EntityType<T> makeEntity(String name, EntityType.EntityFactory<T> factory, EntityCategory category, float width, float height, Supplier<EntityModel<T>> model) {
        ...
        dinosaurs.add(() -> {
            ...
            EntityRendererRegistry.INSTANCE.register(entityType, (entityRenderDispatcher, context) -> new PrehistoricRenderer<>(entityRenderDispatcher, model.get(), name));
            Registry.register(Registry.ITEM, new Identifier("pe", "spawn_egg"), new SpawnEggItem(entityType, 0x0DA70B, 0x73420E, new Item.Settings().group(ItemGroup.MISC)));
        });
